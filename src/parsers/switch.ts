...
import { SyntaxError, UnhandledError, removeEmptyWords, generateAutoGeneratedFnName, isNext, convertBlock, converted, indentCount } from "../convert";
 ...
 */
function convertSwitch(tokens: BaseToken[], insertIndex: number, assigningType: null | true | Type, shouldStatic: boolean, indentLevel: number, changeToYieldReturn = false): { start: number, end: number }[] | false {
  if (tokens.length === 0) {
    return false;
  }
  ...
  const convertStartIndex = tokens.findIndex(token => token.tokenId === removed[i].tokenId);
  const convertEndIndex = tokens.findIndex(token => token.tokenId === removed[leftBraceIndex - 1].tokenId);
  ...
  if (assigningType === null) {
    ...
    while (true) {
      ...
      if (isBlock) {
        ...
        const convertStartIndex = tokens.findIndex(token => token.tokenId === removed[i + 1].tokenId);
        const convertEndIndex = tokens.findIndex(token => token.tokenId === removed[rightBraceIndex].tokenId);
