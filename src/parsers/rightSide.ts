...
import { SyntaxError, UnhandledError, isNext, isMethodKeyword, operators, removeEmptyWords, isExprKeyword, convertBlock, converted, indentCount, assignmentOperators, decorations, changeKind, Kind, BaseDecoration } from "../convert";
...
const canNextToFnOperators = new Set(['(', ')', '{', '}', '++', '--', '&', '^', '!']);
 ...
 */
function convertRightSide(tokens: BaseToken[], insertIndex: number, assigningType: null | true | Type, shouldStatic: boolean, indentLevel: number, testingFnTokens: { index: number, tokenId: number, originalKind: Kind | undefined }[] = []): {
  endAt: number;
  isConst: boolean;
  insertedAutoGeneratedFn: { start: number, end: number }[] | false;
  testingFnTokens: { index: number, tokenId: number, originalKind: Kind | undefined }[];
} {
  let checkIsFnTokenResult: boolean = false;
  const originalConverted: string[] = [];
  for (let item of converted)
    originalConverted.push(item);
  const originalDecorations: BaseDecoration[] = [];
  for (let item of decorations)
    originalDecorations.push(item);
  ...
  let spaceIndexes: Set<number> = new Set();

  const originalLastBoundaryToken = lastBoundaryToken;

  let endAt: number = 0;
  let firstError: SyntaxError | UnhandledError | null = null;
  let isFirst = true;
  while (true) {
    try {
      if (isFirst && testingFnTokens.length !== 0)
        throw new UnhandledError();
      else
        endAt = parseExpr(tokens, false, false);
    } catch (e) {
      isFirst = false;
      if (e instanceof SyntaxError || e instanceof UnhandledError) {
        if (testingFnTokens.length === 0)
          throw e;

        firstError ??= e;

        if (tokens.find(x => x.tokenId ===  testingFnTokens[testingFnTokens.length - 1].tokenId)!.kind === testingFnTokens[testingFnTokens.length - 1].originalKind) {
          tokens.find(x => x.tokenId === testingFnTokens[testingFnTokens.length - 1].tokenId)!.kind = testingFnTokens[testingFnTokens.length - 1].originalKind;
          testingFnTokens.pop();
          if (testingFnTokens.length === 0) {
            throw firstError;
          }
        }
        testingFnTokens[testingFnTokens.length - 1].originalKind = tokens.find(x => x.tokenId === testingFnTokens[testingFnTokens.length - 1].tokenId)!.kind;
      } else {
        throw e;
      }
      converted.splice(0, converted.length);
      for (let item of originalConverted)
        converted.push(item);
      decorations.splice(0, decorations.length);
      for (let item of originalDecorations)
        decorations.push(item);
      setLastBoundaryTokenNext = false;
      isConst = true;
      insertedAutoGeneratedFn = false;
      spaceIndexes = new Set();
      lastBoundaryIndex = converted.length;
      lastBoundaryToken = originalLastBoundaryToken;
      continue;
    }
    break;
  }
  if (converted[converted.length - 1] !== ';') {
    const maxSpaceIndex = Math.max(...Array.from(spaceIndexes));
    if (maxSpaceIndex === converted.length)
      spaceIndexes.delete(maxSpaceIndex);
    converted.push(';\r\n');
  }
  ...
  return { endAt, isConst, insertedAutoGeneratedFn, testingFnTokens };

  function checkIsFnToken(tokenId: number, kind: Kind | undefined, text: string) {
    checkIsFnTokenResult = testingFnTokens.some(x => x.tokenId === tokenId) === false && kind === undefined && text.startsWith('M');
    return checkIsFnTokenResult;
  }
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (current.text === ';' || current.text === ',' || current.text === '{' || current.text === '}' || assignmentOperators.has(current.text)) {
        ...
        if (current.text !== ';' && current.text !== ',')
          return i - 1;
        return i;
      } else if (isMethodKeyword(current.kind) || isExprKeyword(current.kind) || checkIsFnToken(current.tokenId, current.kind, current.text)) {
        const originalI = i;
        const isFnToken = checkIsFnTokenResult;
        checkIsFnTokenResult = false;
        ...
        if (current.text === 'default' && isNext(token => token.category === undefined || token.category === 'context_keyword' || token.text === '(', true, i, tokens) === false) {
          ...
          if (current.kind !== 'keyword.block-or-method')
            isConst = false;
          currentMethod = true;
          const isMethod = isMethodKeyword(current.kind) || isFnToken;
          if (isFnToken) {
            testingFnTokens.push({ index: i, tokenId: current.tokenId, originalKind: current.kind });
          ...
          if (tokens[firstArgIndex.index].text === ';') {
            ...
            if (tokens[firstArgIndex.index].category === 'operator') {
              if (canNextToFnOperators.has(tokens[firstArgIndex.index].text) === false)
                throw new SyntaxError(tokens[firstArgIndex.index]);
            }
