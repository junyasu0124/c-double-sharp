...
export { keywords, contextKeywords, kindOfKeywordOrModifier, operators, assignmentOperators, SyntaxError, UnhandledError, indentCount, converted, decorations, changeKind, convert, convertBlock, trimEmptyWords, removeEmptyWords, generateAutoGeneratedFnName, isAccessor, isNext, isLiteralCategory, isMethodKeyword, isExprKeyword };
...
interface BaseDecoration { start: number, end: number, kind: 'fn-args' | 'indexer' | 'fn-type' | 'tuple' | 'generics' };
let indentCount = 2;
...
function convert(tokens: BaseToken[]): { converted: string, decorations: BaseDecoration[], modified: [number, Kind | undefined][] } {
  converted = [];
  decorations = [];
  modified = [];
  ...
  if (converted.length === 0)
    return { converted: '', decorations: [], modified: [] };
  else if (converted[converted.length - 1] === '\r\n')
    return { converted: converted.join(''), decorations, modified };
  else
    return { converted: converted.join('') + '\r\n', decorations, modified };
}

function changeKind(token: BaseToken, kind: Kind | undefined, force = false) {
  if (force === true || token.kind === undefined) {
    token.kind = kind;
    modified.push([token.tokenId, kind]);
  }
}
...
function trimEmptyWords(tokens: BaseToken[]) {
  let start = 0;
  ...
  while (tokens[start].category === 'space' || tokens[start].category === 'line_break' || tokens[start].category === 'comment') start++;
...
function removeEmptyWords(tokens: BaseToken[]) {
