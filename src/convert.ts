...
export type { Kind, BlockType, Category };
export { keywords, contextKeywords, kindOfKeywordOrModifier, operators, assignmentOperators, SyntaxError, UnhandledError, indentCount, converted, convert, convertBlock, trimEmptyWords, removeEmptyWords, generateAutoGeneratedFnName, isAccessor, isNext, isLiteralCategory, isMethodKeyword, isExprKeyword };
...
class SyntaxError implements Error {
  ...
  constructor(token: Token | { content: string, start: number, end: number } | null = null, message: string | null = null) {
    ...
    const { tokenId } = token as Record<keyof Token, unknown>;
...
function convert(tokens: Token[]): string {
